...
def convert_to_isl(text):
    ...
    video_folder = os.path.join(os.getcwd(), "islAnimatedVideos")

    # Append video clips based on the tokenized text
    for text in result:
        video_file = os.path.join(video_folder, f"{text[0]}.mp4")  # Look inside the islAnimatedVideos folder
        if os.path.exists(video_file):
            arg_array.append(VideoFileClip(video_file))
